...
export interface VerifyOtpResponse {
  ...
  export const sendOtpEmail = async (email: string): Promise<SendOtpResponse | undefined> => {
    try {
      const response: AxiosResponse<SendOtpResponse> = await UserAxiosInstance.post(
        '/user/contract/send-otp',
        { email }
      );
  ...
  export const verifyOtp = async (payload: VerifyOtpPayload): Promise<VerifyOtpResponse | undefined> => {
    try {
      const response: AxiosResponse<VerifyOtpResponse> = await UserAxiosInstance.post(
        '/user/contract/verify-otp',
        payload
      );
      return response.data;
    } catch (error) {
...
export const createContract = async (
  data: CreateContractPayload,request_type: "buy" | "borrow",requestId:string
): Promise<CreateContractResponse | undefined> => {
  try {
    const response: AxiosResponse<CreateContractResponse> = await UserAxiosInstance.post(
