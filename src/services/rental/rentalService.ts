...
export const sendOtpEmailForContractReturn = async (
  email: string,
  bookId?: string
): Promise<SendOtpResponse | undefined> => {
  try {
    const response: AxiosResponse<SendOtpResponse> =
      await UserAxiosInstance.post("/user/rental-book/return/send-otp", {
        email,
        bookId,
      });
...
export const verifyOtpForContractReturn = async (
...
export const fetchRentedOutBooksContract = async (params: {
  page: number;
  limit: number;
  filter?: object;
}): Promise<CombinedRentalContracts> => {
...
export const fetchBorrowedBooksContract = async (params: {
  page: number;
  limit: number;
  filter?: object;
}): Promise<CombinedBorrowedContracts0> => {
  const response = await UserAxiosInstance.get(
    "/user/borrower/rental-contract",
    {
      params,
    }
  );
...
export const fetchRentalContractDetails = async (
  ...
  try {
    const response = await UserAxiosInstance.get(
      `/user/rental-contract/details/${rentalId}`
    );
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError) {
    ...
    throw new Error(
      "Unknown error occurred while fetching rental contract details"
    );
  }
};

export const updateRentalContractStatus = async (
  rentalId: string,
  status: string
) => {
  try {
    const response = await UserAxiosInstance.patch(
      `/user/rental-contract/${rentalId}/status`,
      {
        status,
      }
    );
...
export const requestForRenewRentalContract = async (
  rentalId: string,
  renewalDetails: { days: number; amount: number }
) => {
  try {
    const response = await UserAxiosInstance.put(
      `/user/renew-rental-contract/${rentalId}`,
      renewalDetails
    );
...
export const RenewRentalContract = async (
  rentalId: string,
