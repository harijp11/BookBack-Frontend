...
import { fetchOwnerContractRequests, updateContractRequestStatus, type ContractRequest } from "@/services/contractrequest/contractRequestService"
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/Components/ui/card"
import { Button } from "@/Components/ui/button"
import { Badge } from "@/Components/ui/badge"
import { Check, X, BookOpen, User, Clock, } from "lucide-react"
...
export default function OwnerContractRequestHandling() {
  ...
  const loadRequests = async () => {
    ...
    try {
      setLoading(true)
      const data = await fetchOwnerContractRequests(user._id)
  ...
  const handleUpdateStatus = async (conReqId: string, status: string) => {
    try {
      await updateContractRequestStatus({ conReqId, status })

      // Update local state to reflect the change
      setRequests((prev) => prev.map((request) => (request._id === conReqId ? { ...request, status } : request)))
