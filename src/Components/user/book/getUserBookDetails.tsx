...
import { useBookDetails, useRelatedBooks } from "@/hooks/common/useGetBookdetailsMutation"
...
const BookView: React.FC = () => {
  ...
  const isForRent = book.dealTypeId?.name === "For Rent Only" || book.dealTypeId?.name === "For Rent And Sale"
  const isForSale = book.dealTypeId?.name === "For Sale Only" || book.dealTypeId?.name === "For Rent And Sale"
  const hasRentAmount = book.rentAmount !== undefined && book.rentAmount !== null && book.rentAmount !== 0
          ...
          <div className="lg:w-3/5 space-y-8">
            <div className="opacity-0 translate-y-8" ref={descriptionRef}>
              <div className="mb-6 flex items-center gap-3 flex-wrap">
                <span className="px-4 py-1.5 bg-gradient-to-r from-gray-100 to-gray-300 text-gray-800 text-sm font-medium rounded-full transform transition-all duration-300 hover:scale-105 shadow-sm">
              ...
              >
                ...
                <span>{selectedLocation?.address || book.locationName || "Location unavailable"}</span>
              </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                ...
                <div className="bg-gradient-to-r from-gray-50 to-gray-200 p-6 rounded-xl shadow-lg border border-gray-300/50 transform transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                  ...
                  <span className="text-2xl font-bold text-black flex items-end gap-1">
                    {hasRentAmount ? (
                      <>
                        ₹{book.rentAmount}
                        <span className="text-sm font-normal text-gray-500 ml-1">/contract</span>
                      </>
                    ) : (
                      <span className="text-lg font-normal text-gray-500">Not available for rent</span>
                    )}
                  </span>
                </div>
                ...
                <div className="bg-gradient-to-r from-gray-50 to-gray-200 p-6 rounded-xl shadow-lg border border-gray-300/50 transform transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                  <span className="text-sm text-gray-500 block mb-2">Maximum Rental duration</span>
                  <span className="text-2xl font-bold text-black flex items-end gap-1">
                    {hasRentAmount ? (
                      <>
                         <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 transform transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                  ...
                  <span className="text-2xl font-bold text-gray-800 flex items-end gap-1">
                    ...
                    <span className="text-sm font-normal text-gray-500 ml-1">days</span>
                      ...
                      <span className="text-lg font-normal text-gray-500">Not Available</span>
                    )}
                  </span>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                {isForSale && (
                  <button className="flex-1 bg-white text-gray-800 border-2 border-gray-300 px-6 py-4 rounded-xl font-semibold text-lg hover:border-black hover:text-black transition-all duration-300 flex items-center justify-center gap-3 transform hover:-translate-y-1 shadow-md hover:shadow-lg">
                    <DollarSign size={20} />
                    Buy Now
                  </button>
                )}
          ...
          {relatedLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
              {relatedBooks.map((relatedBook) => (
                <Link key={relatedBook._id} to={`/book/${relatedBook._id}`} className="group">
                  <div className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-500 h-full flex flex-col border border-gray-100 transform group-hover:-translate-y-2">
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                      <h3 className="text-gray-800 font-medium mb-2 line-clamp-1 group-hover:text-black transition-colors">
                        {relatedBook.name}
                      </h3>
                      <div className="mt-auto pt-4 flex justify-between items-center">
                        <p className="font-bold text-black">
                          ₹{relatedBook.rentAmount}
                          <span className="text-xs text-gray-500 ml-1">/contract</span>
                        </p>
